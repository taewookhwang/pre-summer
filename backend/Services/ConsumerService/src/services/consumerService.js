...
class ConsumerService {
   ...
   * @returns {Promise<Array>} List of service categories with subcategories
   */
  ...
  /**
   * Get list of services for a specific category or subcategory with pagination
   * @param {String} categoryId - The category ID
   * @param {String} subcategoryId - The subcategory ID (optional)
      ...
      const result = await sequelize.transaction(async (transaction) => {
        ...
        if (reservationData.serviceOptions && Array.isArray(reservationData.serviceOptions)) {
          ...
          optionsPrice = serviceOptions.reduce((total, option) => {
            const quantity = reservationData.serviceOptions.find(opt => opt.option_id === option.id)?.quantity || 1;
  ...
  async getUserReservations(userId, filters = {}, page = 1, limit = 20) {
    try {
      ...
      const reservations = await Reservation.findAll({
        ...
        include: [{
          model: Service,
          attributes: ['id', 'name', 'shortDescription', 'description', 'basePrice', 'duration', 'categoryId', 'subcategoryId']
        }],
    ...
    try {
      ...
      const result = await sequelize.transaction(async (transaction) => {
        ...
        if (updateData.coordinates) {
          if (updateData.coordinates.latitude) reservation.latitude = updateData.coordinates.latitude;
          if (updateData.coordinates.longitude) reservation.longitude = updateData.coordinates.longitude;
        }
        if (updateData.specialInstructions) reservation.specialInstructions = updateData.specialInstructions;
        ...
        if (updateData.serviceOptions) {
          ...
          optionsPrice = serviceOptions.reduce((total, option) => {
            const quantity = updateData.serviceOptions.find(opt => opt.option_id === option.id)?.quantity || 1;
            return total + (option.price * quantity);
          }, 0);
  ...
  async updateReservationStatus(reservationId, userId, status) {
    try {
      const allowedStatuses = ['pending', 'searching_technician', 'technician_assigned', 'in_progress', 'completed', 'cancelled'];
